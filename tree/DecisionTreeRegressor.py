import numpy as np
from metrics import squared_deviation, absolute_deviation
...
class DecisionTreeRegressor:
    ...
    def __init__(self, max_depth=None, min_samples_split=2, criterion_name='gini'):
    ...
    def dfs(self, node: Node, X):
        if node.left_child is None and node.right_child is None:
            # Aggregate the target values of the samples on this vertex
            raise NotImplementedError('Implement what to do in the leaves')
    ...
    def predict(self, X):
        """
        Predict the target value of the given data
        ...
        Returns
        -------
        y_predicted : np.array of type float with shape (n_objects, 1)
            Column vector of target values in regression

        """

        y_pred = np.array([self.dfs(self.root, obj) for obj in X])
        return y_pred
